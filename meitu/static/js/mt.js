...
$(function(){
    $.setCookie = function(name,value,days){ 
        var days = days || 30, exp = new Date(); //new Date("December 31, 9998"); 
        exp.setTime(exp.getTime() + 86400 * days); 
    ...
    $.fn.render = function(options) { 
        var re = /<%(.+?)?%>/g,
            reExp = /(^( )?(if|for|var|\(?function|else|switch|case|break|{|}))(.*)?/g,
            code = 'var r=[];\n',
            html = this.html(),
            cursor = 0; 
    ...
    $.fn.waterflow = function(options){
        var panel = this,
            options = options || {},
            colMinWidth = options.colMinWidth || 145,
            colMinNum = options.colMinNum || 2,
            colMargin = options.colMargin || 6,
            sideMargin = options.sideMargin || 10,
            colNum = colMinNum,
            colWidth = colMinNum,
            renderTop = [],
            renderLeft = [],
        ...
        function layoutItem(item){
            var img = item.find('img'),
                ...
                n = 0;
            for(; n < colNum; ++n){
                if(renderTop[colIndex] > renderTop[n]){
                    colIndex = n;
                }
            }
            //渲染图片
            ...
            if(!isNaN(width) && !isNaN(height)){
                ...
                if(width > 0 && height > 0){
                    imgHeight = Math.floor(height * (colWidth / width));
                }
            }
            img.css({width: colWidth, height: imgHeight});
            item.css({width: colWidth, top: renderTop[colIndex], left: renderLeft[colIndex]});
                        renderTop[colIndex] += item.height() + colMargin;
            if(renderTop[colIndex] > panelHeight){
                panelHeight = renderTop[colIndex] + sideMargin;
            }
        ...
        function layout(){
            var panWidth = panel.width(),
                items = panel.children('.item'),
                colIndex = 0;
            if(panWidth >= colMinWidth * colMinNum + sideMargin * 2){
                colNum = Math.floor((panWidth - sideMargin * 2 + colMargin) / (colMinWidth + colMargin));
            }
            colWidth = Math.floor((panWidth - colMargin * (colNum - 1) - sideMargin * 2) / colNum);
            //sideMargin = Math.floor((panWidth - colWidth * colNum) / 2);
            //初始化布局相关数据
            panelHeight = 0;
            for(; colIndex < colNum; ++colIndex){
                renderTop[colIndex] = sideMargin;
                renderLeft[colIndex] = sideMargin + colIndex * (colMargin + colWidth);
            }
            //重新布局瀑布流元素
    ...
    $.fn.slider = function(options){
        var $win = $(window),
            panel = this,
            papers = this.children(),
            touchX = 0,
            touchY = 0,
            offsetX = 0,
            currentIndex = 0,
            slideDirection = '',
            onShowPaper = options.onShowPaper,
            onTapPaper = options.onTapPaper,
            onResize = options.onResize;

        function setTransformX(pixel, duration){
            panel.css({
        ...
        // 移动时判断是左滑还是右滑，滑动距离在|4|px之内，算点击事件
        function touchMoveHandler(e){
            ...
            if (typeof e.touches != 'undefined') {
                ...
                moveY = e.touches[0].pageY - touchY;
            } else {
                moveX = e.pageX - touchX;
                moveY = e.pageY - touchY;
            }
        ...
        function touchEndHandler(e){
            if ('left' == slideDirection){
                if(!showPaper(currentIndex + 1, 'left')){
                    // 设置滑动动画
                    setTransformX(offsetX, 500);
                }
            } else if ('right' == slideDirection){
                if(!showPaper(currentIndex - 1, 'right')) {
                    // 设置滑动动画
                    setTransformX(offsetX, 500);
                }
            } else if(typeof onTapPaper === 'function'){
                onTapPaper.call(panel, currentIndex);
            }
        }
        function showPaper(index, direction){
            function slide(){
                var duration = direction ? 500 : 0;
                offsetX = -panel.width() * index;
                currentIndex = index;

                setTransformX(offsetX, duration);
                $(papers[index % 3]).css('left', (index - 1) * 100 + '%');
                $(papers[(index + 2) % 3]).css('left', (index + 1) * 100 + '%');
                $(papers[(index + 1) % 3]).css('left', index * 100 + '%');
            }
            if(typeof onShowPaper === 'function'){
                if(onShowPaper.call(panel, index, direction)){
                    slide();
                    return true;
                }
                return false;
        ...
        function reset(){
            ...
            papers.each(function(i, paper){
                $(paper).css('left', (i - 1) * 100 + '%').empty();
            });
        }
        function bind(){
            panel.on('touchstart', touchStartHandler)
            .on('touchmove', touchMoveHandler)
            .on('touchend', touchEndHandler);
            $(window.document.body).on('touchmove', stopDefaultEvt, false);
        }
        bind();
        return {
            show: showPaper,
            papers: function(index){
                return papers[index];
            },
            index: function(){
                return currentIndex;
...
$(function(){
    window.MtCatViewer = function(options){
        var $itemTpl = $('#cat_item_tpl'),
            $tip = $('#tip'),
            page = 1,
            options = options || {},
            ...
            data,
            scrollTimer,
            resizeEv = ('onorientationchange' in window) 
                ? 'orientationchange' : 'resize';

        function load(cid){
            var url = [apiPrefix, 'albums', cid, page + '.json'].join('/');
            categoryId = cid;
            data = [];
            $tip.html('一大波妹子准备扑向你…');
            ga('send', 'event', 'category', cid, page);
            //ajax请求数据
            $.getJSON(url, function(json){
                if(json.Status === 200){
                    if(Array.isArray(json.Data)){
                        ...
                        $tip.html('加载图片出错，请尝试其他分类！');
                    }
                } else{
                    $tip.html('服务器出错，请刷新重试！');
                }
            });
        }
        ...
        function bind(){
            ...
            $(window).on('scroll', function(e){
                ...
                scrollTimer = setTimeout(function() {
                    var bottom = window.document.body.clientHeight - window.scrollY - window.innerHeight;
                    if(bottom < 100){
                        load(categoryId);
                    }
                }, 200);
            }).on(resizeEv, function(){
                if(resizeTimer){
        ...
        function loadItems(){
            ...
            function loadItem(){
                if(++i < items.length){
                    waterflow.loadItem(items[i], loadItem);
                }
                if(scrollTimer){
                    clearTimeout(scrollTimer);
                }
            ...
            for(var n = 0; n < data.length; ++n){
                items.push($($itemTpl.render(data[n])));
            }
            loadItem();
        }
    ...
    window.MtAlbumViewer = function(options){
        var $itemTpl = $('#slider_item_tpl'),
            $body = $(document.body),
            $header = $('header'),
        ...
        function createPlayer(uri){
            var str = [], $player = $('#player');
            str.push('<object id="player" name="QvodPlayer" width="100%" height="100%" ');
            str.push('classid="clsid:F3D0D36F-23F8-4682-A195-74C92B03D4AF"><param name="URL" value="');
            str.push(uri);
            str.push('"/><param name="Autoplay" value="1"/><param name="Showcontrol" value="0"/>');
            str.push('<embed width="0" height="0" url="');
            str.push(uri);
            str.push('" type="application/qvod-plugin" autoplay="1" showcontrol="0"/></object>');
            if($player.length > 0){
                $player.remove();
            }
        ...
        function loadPic(paper, data){
            var $paper = $(paper),
                $item = $($itemTpl.render(data)),
                $img = $item.find('img.pic'),
                loadImg = new Image(),
                paperWidth = $paper.width(),
                paperHeight = $paper.height();
            $item.hide();
            $paper.html($item).removeClass('default').addClass('loading');
            loadImg.src = data.Url;
            loadImg.onerror = function(){
                
                $paper.removeClass('loading').addClass('default');
            }
            loadImg.onload = function(){
                var ratio = loadImg.width / loadImg.height,
                    winRatio =  paperWidth / paperHeight
                    itemWidth = paperHeight * ratio,
                    itemHeight = paperWidth / ratio;
                $img.attr('src', data.Url);
                $paper.removeClass('loading');
                if(ratio > winRatio){
                    $item.css({
                        width: '100%',
                        height: itemHeight,  
                        top: (paperHeight - itemHeight) / 2 + 'px',
                        left: 0
                    }).show();
                }
                else{
                    $item.css({
                        width: itemWidth,
                        height: '100%',
                        left: (paperWidth - itemWidth) / 2 + 'px',
                        top: 0
                    }).show();
                }
            };
        ...
        function loadCurrent(index, pushState){
            var paper = slider.papers((index + 1) % 3),
                image = data.Images[index],
                title = image.Title || data.Title;
            image.PlayLink = image.PlayLink || data.PlayLink;
            loadPic(paper, image);
            $count.show().html((index + 1) + '/' + count);
            $title.html(title);
        }
        slider = $('#slider').slider({
            onShowPaper: function(index, direction){
                var last, current, next, paper;
                if(direction === 'right' && index < 0){
                    if(data.PrevAlbumId > 0){
                        ga('send', 'event', 'album', 'previous', data.Id);
                        window.location.href = data.PrevAlbumId + '.html';
                    }
                    return false;
                } else if(direction === 'left' && index > count){
                    if(data.NextAlbumId > 0){
                        ga('send', 'event', 'album', 'next', data.Id);
                        window.location.href = data.NextAlbumId + '.html';
                    }
                    return false;
                }
                if(index <= count && index >= 0){
                    if(index < count){
                        //载入当前图片
                        ga('send', 'event', 'image', data.Id, index + '/' + count);
                        loadCurrent(index, true);
                    }
                    
                    paper = slider.papers(index % 3);
                    if(index === 0){
                        $(paper).empty();
                    } else {
                        //载入前一个图片
                        loadPic(paper, data.Images[index - 1]);
                    }

                    paper = slider.papers((index + 2) % 3);
                    if(index < count - 1){
                        //载入后一个图片
                        loadPic(paper, data.Images[index + 1]);
        ...
        function loadRecommend(paper){
            var $paper = $(paper).empty().removeClass('loading').removeClass('default'),
                $recTpl = $('#slider_recommend_tpl');
            $paper.html($recTpl.render(data));
        }

        function load(aid){
            var url = [apiPrefix, 'images', aid + '.json'].join('/');
            $.getJSON(url, function(json){
                if(json.Status === 200){
                    data = json.Data || {};
                    ...
                    if(!!data.PlayUri){
                        $play.show().on('click tap', function(){
                            createPlayer(data.PlayUri);
                        });
                    }
                    //$back.attr('href', '/category/' + data.CategoryId + '.html');
                    if(Array.isArray(data.Images)){
                        count = data.Images.length;
                        slider.reset();
                        slider.show(0);
                        ga('send', 'event', 'album', 'loaded', data.Id);
                    }
                    else{
                        ga('send', 'event', 'album', 'error_' + json.Status, data.Id);
                    }
                }
                else{
                    ga('send', 'event', 'album', 'error_' + json.Status, data.Id);
